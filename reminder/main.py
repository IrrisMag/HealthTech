...
HARDCODED_TRANSLATIONS = {
    "appointment": {
        "en": (
            "Hello{name}! 😊 Just a reminder that you have an "
            "appointment on {date} at {time} in room {room} with "
            "Dr. {doctor} at Douala General Hospital. "
            "We're looking forward to seeing you!"
        ),
        "fr": (
            "Bonjour{name} ! 😊 Petit rappel : vous avez un "
            "rendez-vous le {date} à {time} dans la salle {room} "
            "avec le Dr {doctor} à l'Hôpital Général de Douala. "
            "Nous avons hâte de vous voir !"
        ),
        "bassa": (
            "Mbolo{name} ! 😊 Nda'a, o bɛ́ nɛ́ rendez-vous bɛ́ {date} "
            "nɛ {time} na salle {room} nɛ́ Docteur {doctor} "
            "na Douala General Hospital. Nda'a, o bɛ́ nɛ́ yɔ́kɔ́ !"
        ),
        "ewondo": (
    ...
    "medication": {
        "en": (
            "Hi{name}! 🌟 It's time to take your medication: "
            "{medication_name}, {dosage}. Take care of yourself!"
        ),
        "fr": (
            "Bonjour{name} ! 🌟 Il est temps de prendre votre "
            "médicament : {medication_name}, {dosage}. "
            "Prenez soin de vous !"
        ),
        "bassa": (
            "Mbolo{name} ! 🌟 Nda'a, o bɛ́ nɛ́ yɔ́kɔ́ médicament : "
            "{medication_name}, {dosage}. Nda'a, o bɛ́ nɛ́ yɔ́kɔ́ !"
        ),
        "ewondo": (
            "Mbembe{name} ! 🌟 Nda'a, o bɛ́ nɛ́ yɔ́kɔ́ médicament : "
            "{medication_name}, {dosage}. Nda'a, o bɛ́ nɛ́ yɔ́kɔ́ !"
        ),
        "nguemba": (
            "Wɛ́{name} ! 🌟 Nda'a, wɛ́ nɛ yɔ́kɔ́ médicament : "
            "{medication_name}, {dosage}. Nda'a, wɛ́ nɛ yɔ́kɔ́ !"
        )
    }
}
...
class AppointmentReminderRequest(BaseModel):
    ...
    patient_language: str = Field(
        ..., pattern="^(fr|en|bassa|ewondo|nguemba)$"
    )
...
class AppointmentReminderUpdate(BaseModel):
    ...
    patient_language: Optional[str] = Field(
        None, pattern="^(fr|en|bassa|ewondo|nguemba)$"
    )
...
def translate_message(message_type: str, message: str, lang: str,
                      **kwargs) -> str:
...
def send_notification(to: str, body: str):
    try:
        ...
        if resp.status_code != 200:
            raise Exception(
                f"Notification service error: {resp.text}"
            )
...
def schedule_notification(to: str, body: str, send_time: datetime):
    ...
    scheduler.add_job(
        send_notification, 'date', run_date=send_time, args=[to, body],
...
def reschedule_appointment_notifications(reminder_data: dict):
    ...
    base_msg = (
        reminder_data.get("message") or
        HARDCODED_TRANSLATIONS["appointment"]["en"]
    )
    ...
    if two_days_before > datetime.now(tz):
        schedule_notification(
            reminder_data["patient_phone"], msg, two_days_before
        )
    ...
    if two_hours_before > datetime.now(tz):
        schedule_notification(
            reminder_data["patient_phone"], msg, two_hours_before
        )
        ...
        for med in reminder_data["medications"]:
            for t in med["times"]:
                ...
                send_time = tz.localize(
                    datetime.combine(current, dt_time(hour, minute))
                )
                base_msg = (
                    reminder_data.get("message") or
                    HARDCODED_TRANSLATIONS["medication"]["en"]
                )
                msg = translate_message(
                    "medication", base_msg,
                    reminder_data["patient_language"],
                ...
                if send_time > datetime.now(tz):
                    schedule_notification(
                        reminder_data["patient_phone"], msg, send_time
                    )
...
def create_appointment_reminder(reminder: AppointmentReminderRequest):
    ...
    # Save to DB
    reminder_doc = {
        "type": "appointment", **reminder.dict(), "created_at": datetime.now()
    }
...
def create_medication_reminder(reminder: MedicationReminderRequest):
    ...
    reminder_doc = {
        "type": "medication", **reminder_data, "created_at": datetime.now()
    }
    result = reminders.insert_one(reminder_doc)
    ...
    while current <= reminder.end_date:
        for med in reminder.medications:
            for t in med.times:
                ...
                msg = translate_message(
                    "medication", base_msg,
                    reminder.patient_language, name=reminder.patient_name,
                    medication_name=med.name, dosage=med.dosage
                )
                # Send now if today and time is in the future
                if (current == date.today() and
                        send_time > datetime.now(tz)):
                    send_notification(reminder.patient_phone, msg)
                # Schedule for future
                if send_time > datetime.now(tz):
                    schedule_notification(
                        reminder.patient_phone, msg, send_time
                    )
        current += timedelta(days=1)
...
def list_reminders(
        ...
        reminder_type: Optional[str] = Query(
            None,
            description="Filter by reminder type (appointment/medication)"
        ),
        limit: int = Query(
            100, description="Maximum number of reminders to return"
        ),
    ...
    cursor = reminders.find(query).skip(skip).limit(limit).sort(
        "created_at", -1
    )
    result = []
    for doc in cursor:
...
def get_reminder(reminder_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid reminder ID format"
        )
...
def update_appointment_reminder(
    reminder_id: str, update_data: AppointmentReminderUpdate
):
    ...
    existing_reminder = reminders.find_one(
        {"_id": object_id, "type": "appointment"}
    )
    if not existing_reminder:
        raise HTTPException(
            status_code=404, detail="Appointment reminder not found"
        )
...
def update_medication_reminder(
    reminder_id: str, update_data: MedicationReminderUpdate
):
    ...
    # Get existing reminder
    existing_reminder = reminders.find_one(
        {"_id": object_id, "type": "medication"}
    )
    if not existing_reminder:
        raise HTTPException(
            status_code=404, detail="Medication reminder not found"
        )
...
def delete_reminder(reminder_id: str):
    ...
    return {
        "message": "Reminder deleted successfully", "deleted_id": reminder_id
    }
...
def delete_patient_reminders(patient_id: str):
    ...
    patient_reminders = list(
        reminders.find({"patient_id": patient_id})
    )
    ...
    return {
        "message": (
            f"Deleted {result.deleted_count} reminders for patient "
            f"{patient_id}"
        ),
