...
# 🚀 Deploy Track 3 Backend to Railway HealthTech Project
...
echo -e "${BLUE}🚀 DEPLOYING TRACK 3 BACKEND TO RAILWAY HEALTHTECH${NC}"
echo -e "${BLUE}=================================================${NC}"
echo ""
...
TRACK3_DIR="track3-blood-bank-system"
if [ ! -d "$TRACK3_DIR" ]; then
    echo -e "${RED}❌ Track 3 directory not found: $TRACK3_DIR${NC}"
...
echo -e "${YELLOW}🛠️  Creating Railway project configuration...${NC}"
...
# Create main.py - Complete Track 3 Backend
cat > main.py << 'EOF'
"""
...
if [ ! -f ".railway" ]; then
    echo -e "${YELLOW}Creating new Railway project in HealthTech team...${NC}"
    railway init --name "track3-blood-bank-backend"
else
    echo -e "${GREEN}✅ Already in Railway project${NC}"
fi
...
# Add all files to git (Railway needs git)
git init
git add .
git commit -m "Initial Track 3 backend deployment - AI-Enhanced Blood Bank System"
...
if [ -n "$RAILWAY_URL" ]; then
    echo -e "${GREEN}🎉 TRACK 3 BACKEND DEPLOYED SUCCESSFULLY!${NC}"
    echo -e "${GREEN}🌐 Backend URL: ${RAILWAY_URL}${NC}"
    echo ""
    echo -e "${BLUE}📋 DEPLOYMENT SUMMARY:${NC}"
    echo -e "${BLUE}=====================${NC}"
    echo -e "🔗 Backend URL: ${RAILWAY_URL}"
    echo -e "🗄️  Database: MongoDB Atlas (healthtech_track3)"
    echo -e "🌐 DHIS2: https://play.im.dhis2.org/stable-2-42-1"
    echo -e "🤖 AI Features: ARIMA Forecasting, XGBoost, Optimization"
    echo -e "📊 Real-time: Live inventory, donor management, requests"
    echo ""
    echo -e "${BLUE}🔍 API ENDPOINTS:${NC}"
    echo -e "• Health Check: ${RAILWAY_URL}/health"
    echo -e "• DHIS2 Test: ${RAILWAY_URL}/dhis2/test"
    echo -e "• Donors: ${RAILWAY_URL}/donors"
    echo -e "• Inventory: ${RAILWAY_URL}/inventory"
    echo -e "• Requests: ${RAILWAY_URL}/requests"
    echo -e "• Forecasting: ${RAILWAY_URL}/forecast/{blood_type}"
    echo -e "• Analytics: ${RAILWAY_URL}/analytics/performance"
    echo ""
    echo -e "${GREEN}✅ Track 3 backend is now live and operational!${NC}"
else
    echo -e "${YELLOW}⚠️  Deployment in progress. Check Railway dashboard for status.${NC}"
...
echo -e "✅ Inventory optimization recommendations"
